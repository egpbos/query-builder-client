...
# select all ancestors from entities where name is like <name>
WITH tblParent AS
(
    SELECT * FROM entities WHERE id IN (SELECT id FROM entities WHERE name LIKE <name>)
    UNION ALL
    SELECT entities.* FROM entities JOIN tblParent  ON entities.id = tblParent.parent_id
)
SELECT * FROM tblParent;

# select all ancestors from instances where name is like <name>
WITH tblParent AS
(
    SELECT * FROM entities WHERE id IN (SELECT entity_id FROM instances WHERE name LIKE <name>)
    UNION ALL
    SELECT entities.* FROM entities JOIN tblParent  ON entities.id = tblParent.parent_id
)
SELECT * FROM tblParent;

# select all ancestors from entities and/or instances where name is like <name>
WITH tblParent AS
(
    SELECT * FROM entities WHERE id IN (
        SELECT id FROM entities WHERE name LIKE <name>
        UNION ALL
        SELECT entity_id FROM instances WHERE name LIKE <name>)
    UNION ALL
    SELECT entities.* FROM entities JOIN tblParent  ON entities.id = tblParent.parent_id
)
SELECT * FROM tblParent;
